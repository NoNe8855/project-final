...
DELETE
  FROM USER_ROLE;
DELETE
  FROM CONTACT; -- Сначала удаляем из CONTACT, так как он ссылается на PROFILE
DELETE
  FROM PROFILE; -- Потом из PROFILE, так как он ссылается на USERS

DELETE
  FROM ACTIVITY;
--alter sequence ACTIVITY_ID_SEQ restart with 1;
DELETE
  FROM TASK_TAG; -- Добавил DELETE для TASK_TAG, если он ссылается на TASK
DELETE
  FROM USER_BELONG; -- Добавил DELETE для USER_BELONG, если он ссылается на TASK/SPRINT/PROJECT или USERS
DELETE
  FROM ATTACHMENT; -- Добавил DELETE для ATTACHMENT, если он ссылается на TASK/SPRINT/PROJECT или USERS
DELETE
  FROM TASK;
...
DELETE
  FROM MAIL_CASE; -- Добавил DELETE для MAIL_CASE
DELETE
  FROM REFERENCE; -- Добавил DELETE для REFERENCE
...
INSERT INTO PROJECT (ID, code, title, description, type_code, parent_id) -- Добавил ID для явности
VALUES (1, 'PR1', 'PROJECT-1', 'test project 1', 'task_tracker', NULL),
       (2, 'PR2', 'PROJECT-2', 'test project 2', 'task_tracker', 1);

INSERT INTO SPRINT (ID, status_code, startpoint, endpoint, code, project_id) -- Добавил ID для явности
VALUES (1, 'finished', '2023-05-01 08:05:10', '2023-05-07 17:10:01', 'SP-1.001', 1),
       ...
       (5, 'active', '2023-05-10 08:06:00', NULL, 'SP-2.001', 2), -- ID для SPRINT.ID в TASK должен совпадать
...
VALUES
    -- Начинаем ID со 101, чтобы не конфликтовать с автогенерацией H2 (которая начнется с 1)
    ...
    -- (104, 1, 2, 1, 'task_developer', '2023-06-13 12:35:00', null), -- Строка 4 была закомментирована, оставляем так. Если раскомментировать, ID тоже должен быть >100
...
ALTER TABLE PROJECT ALTER COLUMN ID RESTART WITH 3; -- Если для PROJECT максимальный ID был 2
ALTER TABLE USERS ALTER COLUMN ID RESTART WITH 5;   -- Если для USERS максимальный ID был 4
-- и так далее для TASK, ACTIVITY, USER_BELONG, ATTACHMENT, MAIL_CASE, REFERENCE

-- Порядок DELETE:
-- Сначала таблицы, которые ссылаются на другие (многие-ко-многим или detail)
-- Потом таблицы, на которые ссылаются
-- Если есть циклические зависимости, которые не решаются через ON DELETE CASCADE,
-- может потребоваться временно отключать ограничения (не рекомендуется для data.sql)
-- или тщательно продумывать порядок.
-- Я добавил DELETE для всех таблиц, которые ты создаешь, в более-менее безопасном порядке.
-- Убедись, что FK настроены с ON DELETE CASCADE в changelog-test.sql, где это возможно,
-- чтобы упростить DELETE операции.