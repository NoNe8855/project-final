...
-- Файл: src/test/resources/db/changelog-test.sql (ДЛЯ H2)

--changeset kmpk:init_schema_h2_v4 -- Новый ID, если вносим существенные правки
...
CREATE TABLE CONTACT (
                         ...
                         "VALUE" VARCHAR(256) NOT NULL, -- Экранировано
                         PRIMARY KEY (ID, CODE),
...
CREATE TABLE PROJECT (
                         ID          BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
                         CODE        VARCHAR(32)   NOT NULL UNIQUE, -- Упрощено определение UNIQUE
...
CREATE TABLE SPRINT (
                        ID          BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
                        CODE        VARCHAR(32)   NOT NULL, -- Сразу CODE
...
CREATE TABLE TASK (
                      ID            BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
                      TITLE         VARCHAR(1024) NOT NULL,
                      DESCRIPTION   VARCHAR(4096) NULL,     -- <--- СДЕЛАНО NULLABLE
                      TYPE_CODE     VARCHAR(32)   NOT NULL,
                      STATUS_CODE   VARCHAR(32)   NOT NULL,
    -- PRIORITY_CODE VARCHAR(32)   NOT NULL, -- Убраны, так как они удалялись в оригинальном changelog
    -- ESTIMATE      INTEGER,
    -- UPDATED       TIMESTAMP,
                      PROJECT_ID    BIGINT        NOT NULL,
                      SPRINT_ID     BIGINT,
...
--changeset kmpk:populate_data_h2_v3
-- (Все INSERT'ы из твоего populate_data)
-- ...

--changeset ishlyakhtenkov:change_task_status_reference_h2_v3
-- (DELETE и INSERT'ы для TASK_STATUS REFERENCE)
-- ...

--changeset valeriyemelyanov:change_user_type_reference_h2_v3
-- (DELETE и INSERT для USER_TYPE REFERENCE)
-- ...

--changeset apolik:refactor_reference_aux_h2_v3
-- (DELETE и INSERT для TASK_TYPE REFERENCE с AUX)
-- ...

-- Остальные changeset'ы (gkislin:change_backtracking_tables, gkislin:users_add_on_delete_cascade, ishlyakhtenkov:change_UK_USER_BELONG)
-- НЕ НУЖНЫ для этой H2-версии, так как их логика учтена в init_schema_h2_v3 или упрощена.